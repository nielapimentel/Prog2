...
void TrocaSeAcharMenor(int * vet, int tam, int * paraTrocar){
    ...
    for (int i=0; i<tam; i++){
        if (vet[i] < *paraTrocar){ //nao entendi pq tem que ser sinal de > ao inves de menor aqui
            aux = *paraTrocar;
            *paraTrocar = vet[i];
            vet[i] = aux;
        }
...
void OrdeneCrescente(int * vet, int tam){
    for (int i=0; i<tam; i++){ //quando vc ordena a penultima "casa", vc automaticamente ordena a ultima
